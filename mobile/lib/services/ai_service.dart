...
class AIService {
  ...
  final String _cohereKey =
      dotenv.env['COHERE_API_KEY'] ?? '6A1SzbNqSpmBRTfZzVMt8k7fj653gSy8TipWIzZO';
  ...
  Future<List<Map<String, dynamic>>> extractMarkingGuideFromText(String text,
      {bool useGroq = true}) {
  ...
  Future<Map<String, dynamic>> _hybridGradeAnswer({
    ...
    final keywordScore = keywords.isEmpty
        ...
        : keywords
                .where((kw) =>
                    studentAnswer.toLowerCase().contains(kw.toLowerCase()))
                .length /
            keywords.length;
    ...
    final combinedScore = (0.4 * similarity +
            0.3 * keywordScore +
  ...
  Future<Map<String, String>> extractAnswersFromText(
    ...
    final prompt = """
Extract student answers from exam text. Return JSON format: { "1": "answer to question 1", "2": "answer to question 2" }
...
""";
  ...
  Future<List<Map<String, dynamic>>> _extractMarkingGuide(
    ...
    final prompt = """
Extract marking guide as JSON array: [ {"question": "...", "modelAnswer": "...", "marks": X}, {"question": "...", "modelAnswer": "...", "marks": X} ]
...
""";
  ...
  Future<String> _callCohereAPI(String text) async {
    ...
    final prompt = """
Analyze this exam script and provide marks and feedback for each question you identify. Format each as: Q: [question] A: [answer] Mark: X/Y Feedback: [feedback]
