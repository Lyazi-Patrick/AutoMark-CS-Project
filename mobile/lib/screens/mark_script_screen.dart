...
class _MarkScriptScreenState extends State<MarkScriptScreen> {
      ...
      for (var guide in guideAnswers) {
        if (guide.containsKey('question') && guide.containsKey('modelAnswer')) {
          ...
          if (studentAnswer.toLowerCase().trim() ==
              modelAnswer.toLowerCase().trim()) {
            score += guide['marks'] as int;
          }
        } else if (guide.containsKey('rawText')) {
          // Fallback: check if rawText is  really present in OCR text
  ...
  Future<void> _aiMark(List<Map<String, dynamic>> guideAnswers) async {
    ...
    try {
      ...
      final structuredAnswers =
          guideAnswers
              .where(
                (e) =>
                    e.containsKey('question') && e.containsKey('modelAnswer'),
              )
              .map(
                (e) => {
                  "question": e['question'],
      ...
      final rawEntries =
          guideAnswers.where((e) => e.containsKey('rawText')).toList();
      ...
      for (var rawEntry in rawEntries) {
        ...
        rawDetails.add({
          ...
          "feedback":
              found
                  ? "Raw text matched in student answers."
                  : "Raw text not found.",
        });
      }
      ...
      final combinedDetails = List<Map<String, dynamic>>.from(
        result['details'] ?? [],
      )..addAll(rawDetails);
  ...
  Future<void> _saveResult(
    ...
    final percentage =
        total != null && total > 0 ? ((score / total) * 100).round() : 0;
    ...
    if (id != null) {
      final scriptDoc = FirebaseFirestore.instance
          .collection('scripts')
          .doc(id);
      ...
      if (docSnapshot.exists && docSnapshot.data()?['userId'] == userId) {
        ...
        _showSnackBar(
          "⚠ Cannot update script: Permission denied.",
          isError: true,
        );
      }
    }
